...
It provides nine tools: list the tribunals, read any skill file, fetch a tribunal's foundational texts, verify a citation (the server detects the tribunal from the citation and returns its authoritative sources and verification workflow), search the jurisprudence, retrieve a primary-source document (with PDF text extraction, and the fallback ladder when an official site blocks a direct fetch), parse and validate a citation's form against the tribunal's documented scheme (`resolve_citation`), check a claimed quotation against the authoritative source itself (`verify_quote` — the tool reports verbatim, close-match, or not-found, and never confirms a quote the source does not contain), and route a research query to the right Tier 1 search entry points (`search_sources`). No tool produces a citation from memory.
